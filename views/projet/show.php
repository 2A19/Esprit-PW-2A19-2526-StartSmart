...
<div class="projet-detail-container" style="max-width: 900px; margin: 0 auto;">
    ...
    <div class="projet-hero" style="background: linear-gradient(135deg, #1e3c72, #2a5298); color: white; border-radius: 16px; padding: 40px; margin-bottom: 30px; position: relative; overflow: hidden; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">
        <div style="position: relative; z-index: 2; display: flex; justify-content: space-between; align-items: flex-start;">
            <div style="flex: 1;">
                <span id="projetCategory" style="background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.4); padding: 5px 12px; border-radius: 20px; font-size: 0.85em; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">—</span>
                <h1 id="projetTitle" style="margin: 20px 0 10px 0; font-size: 3em; font-weight: 800; line-height: 1.1; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">Loading…</h1>
                <p id="projetRef" style="margin: 0; font-size: 1.1em; opacity: 0.8; font-family: monospace;">Ref: —</p>
                <div id="projetAuthor" style="margin-top: 15px; display: inline-flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.1); padding: 5px 15px 5px 5px; border-radius: 30px; cursor: pointer;">
                    <div id="projetAuthorAvatar" style="width: 35px; height: 35px; background: white; color: #1e3c72; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.2em;">U</div>
                    <span id="projetAuthorName" style="font-weight: bold; font-size: 1.1em;">Utilisateur</span>
                </div>
            </div>
    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 30px; margin-bottom: 40px;">
        <div>
            <div style="background: white; border-radius: 16px; padding: 40px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 30px;">
                ...
                <div id="competencesList" style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px dashed #ecf0f1; display:none;"></div>
                <div id="projetDescription" style="font-size: 1.15em; line-height: 2; color: #34495e; white-space: pre-wrap; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; letter-spacing: 0.2px;">Loading…</div>
            </div>
            ...
            <div style="background: white; border-radius: 16px; padding: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                ...
                <div class="reactions-buttons" style="display: flex; justify-content: center; gap: 20px; margin-top: 20px;">
                    <div class="reaction-large" id="btnLike_<?php echo $projectId; ?>" style="cursor: pointer; padding: 15px 30px; border-radius: 12px; border: 2px solid #e1e8ed; text-align: center; transition: all 0.2s; background: white; border-color: #e1e8ed;" onclick="toggleProjetReaction(PROJECT_ID, 'LIKE')">
                        ...
                        <div style="font-size: 1.5em; font-weight: bold; color: #2c3e50;" id="like-count_<?php echo $projectId; ?>">0</div>
                    ...
                    <div class="reaction-large" id="btnDislike_<?php echo $projectId; ?>" style="cursor: pointer; padding: 15px 30px; border-radius: 12px; border: 2px solid #e1e8ed; text-align: center; transition: all 0.2s; background: white; border-color: #e1e8ed;" onclick="toggleProjetReaction(PROJECT_ID, 'DISLIKE')">
                        ...
                        <div style="font-size: 1.5em; font-weight: bold; color: #2c3e50;" id="dislike-count_<?php echo $projectId; ?>">0</div>
        ...
        <div>
            <div style="background: white; border-radius: 16px; padding: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 30px;">
                ...
                <div style="margin-bottom: 20px;"><div style="font-size: 0.9em; color: #7f8c8d; text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Fonds Requis</div><div id="budgetValue" style="font-size: 2em; font-weight: 800; color: #e67e22;">0 DT</div></div>
                <div style="margin-bottom: 20px;"><div style="font-size: 0.9em; color: #7f8c8d; text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Gains Estimés</div><div id="gainValue" style="font-size: 2em; font-weight: 800; color: #27ae60;">0 DT</div></div>
                <div id="roiBox" style="background: #f8f9fa; border-radius: 8px; padding: 15px; border-left: 4px solid #27ae60;"><div style="font-size: 0.9em; color: #7f8c8d; font-weight: bold;">ROI Potentiel</div><div id="roiValue" style="font-size: 1.5em; font-weight: bold; color: #27ae60;">0%</div></div>
            </div>

            <div style="background: white; border-radius: 16px; padding: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                <h3 style="margin-top: 0; font-size: 1.3em; color: #2c3e50; border-bottom: 2px solid #f0f2f5; padding-bottom: 10px;">Planning</h3>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;"><div><div style="font-size: 0.85em; color: #7f8c8d; font-weight: bold; margin-bottom: 5px;">LANCEMENT</div><div id="startDate" style="font-weight: 600; color: #2c3e50;">—</div></div><div style="color: #bdc3c7;">➔</div><div style="text-align: right;"><div style="font-size: 0.85em; color: #7f8c8d; font-weight: bold; margin-bottom: 5px;">CLÔTURE</div><div id="endDate" style="font-weight: 600; color: #2c3e50;">—</div></div></div>
            </div>
        </div>
    </div>
    ...
    <div class="comments-section" id="comments" style="background: white; border-radius: 16px; padding: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        <div class="comments-header" style="margin-bottom: 30px; border-bottom: 2px solid #f0f2f5; padding-bottom: 15px;">
            <h3 style="margin: 0; font-size: 1.5em; color: #2c3e50;">💬 Questions & Avis (<span id="commentCount">0</span>)</h3>
...
async function loadProjet() {
    ...
    try {
        ...
        // Basic fields
        document.getElementById('projetCategory').innerText = p.categorie_nom || 'Général';
        document.getElementById('projetTitle').innerText = p.nomprojet || p.name || 'Projet';
        document.getElementById('projetRef').innerText = 'Ref: ' + (p.num || '—');
        document.getElementById('projetAuthorAvatar').innerText = (p.auteur_nom||'U').substring(0,1).toUpperCase();
        document.getElementById('projetAuthorName').innerText = p.auteur_nom || 'Utilisateur';
        document.getElementById('projetAuthor').onclick = ()=>{ window.location.href = 'index.php?controller=profile&action=index&id=' + encodeURIComponent(p.auteur_id); };
        ...
        if (p.competences && p.competences.length) {
            ...
            p.competences.forEach(c=>{ const s=document.createElement('span'); s.style='background:#e8f4fd;color:#2980b9;font-size:13px;padding:6px 12px;border-radius:6px;font-weight:bold;border:1px solid #d4e6f1;margin-right:6px;display:inline-block;'; s.innerText = c.nom || c; comps.appendChild(s); });
        } else { comps.style.display='none'; }

        // Description
        document.getElementById('projetDescription').innerText = p.description || 'Aucune description fournie.';

        // Metrics
        document.getElementById('like-count_' + PROJECT_ID).innerText = p.likes_count || 0;
        document.getElementById('dislike-count_' + PROJECT_ID).innerText = p.dislikes_count || 0;
        document.getElementById('btnLike_' + PROJECT_ID).classList.toggle('liked', p.current_user_reaction === 'LIKE');
        document.getElementById('btnDislike_' + PROJECT_ID).classList.toggle('disliked', p.current_user_reaction === 'DISLIKE');
        document.getElementById('budgetValue').innerText = (p.budget||0) + ' DT';
        document.getElementById('gainValue').innerText = (p.gain||0) + ' DT';
        const roi = p.budget>0 ? (((p.gain||0)-p.budget)/p.budget)*100 : 0; document.getElementById('roiValue').innerText = (roi>0?'+':'')+roi.toFixed(1)+'%';
        document.getElementById('startDate').innerText = p.datedebut ? new Date(p.datedebut).toLocaleDateString() : '—';
        document.getElementById('endDate').innerText = p.datefin ? new Date(p.datefin).toLocaleDateString() : '—';

        // Comments
        renderComments(p.commentsTree || [], p.commentsFlat || []);
        document.getElementById('commentCount').innerText = (p.commentaires_count || (p.commentsFlat? p.commentsFlat.length: 0));

        // Comment form
        renderCommentForm(p.current_user_logged_in, PROJECT_ID);
