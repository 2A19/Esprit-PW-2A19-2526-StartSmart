<div class="projet-detail-container" style="max-width: 800px;">
    ...
    <div style="background: white; border-radius: 12px; padding: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        <form id="formProjetEdit" onsubmit="return validerProjetEdit(event)">
            ...
            <input type="hidden" id="project_id_edit" name="id" value="<?php echo htmlspecialchars($this->projet->id); ?>">
...
<script>
async function validerProjetEdit(e) {
    e.preventDefault();
    ...
    const projectId = urlParams.get('id') || document.getElementById('project_id_edit').value;
    if (!projectId) {
        errorMsg.innerHTML = 'Erreur: ID du projet introuvable.';
        errorMsg.style.display = 'block';
        return false;
    }

    const form = document.getElementById('formProjetEdit');
    const data = new FormData(form);
    const payload = {
        nomprojet: data.get('nomprojet'),
    ...
    try {
        const res = await fetch('public/api.php?controller=projet&action=edit&id=' + encodeURIComponent(projectId), {
            method: 'POST',
        ...
        if (json.success) {
            window.location.href = 'index.php?controller=projet&action=show&id=' + encodeURIComponent(projectId);
        } else {
            errorMsg.innerHTML = json.message || 'Erreur lors de la modification du projet';
            errorMsg.style.display = 'block';
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
    } catch (err) {
