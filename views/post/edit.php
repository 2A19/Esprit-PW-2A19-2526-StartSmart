<div class="projet-detail-container" style="max-width: 800px; margin: 0 auto; padding: 20px;">
    <div style="margin-bottom: 20px;">
        <a href="index.php?controller=post&action=show&id=<?php echo htmlspecialchars($this->post->id_post); ?>" style="color: #3498db; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 5px;">
    ...
    <div style="background: white; border-radius: 16px; padding: 40px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
        <form id="formPostEdit" action="index.php?controller=post&action=edit&id=<?php echo htmlspecialchars($this->post->id_post); ?>" method="POST" onsubmit="return validerPostEdit(event)">
            ...
            <div class="form-group" style="margin-bottom: 25px;">
                ...
                <input type="text" id="titre_post_edit" name="titre" value="<?php echo htmlspecialchars($this->post->titre); ?>" style="width: 100%; padding: 14px; border: 1px solid #dce1e6; border-radius: 8px; font-size: 16px; transition: border-color 0.3s;" onfocus="this.style.borderColor='#3498db'" onblur="this.style.borderColor='#dce1e6'">
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 25px;">
                <div class="form-group">
                    ...
                    <select id="topic_post_edit" name="topic" style="width: 100%; padding: 14px; border: 1px solid #dce1e6; border-radius: 8px; font-size: 15px; background-color: #f8f9fa; cursor: pointer;">
                        <?php foreach ($topics as $t): ?>
                            <option value="<?php echo htmlspecialchars($t); ?>" <?php echo ($this->post->topic == $t) ? 'selected' : ''; ?>><?php echo htmlspecialchars($t); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="statut_post_edit" name="statut" style="width: 100%; padding: 14px; border: 1px solid #dce1e6; border-radius: 8px; font-size: 15px; background-color: #f8f9fa; cursor: pointer;">
                        <option value="actif" <?php echo ($this->post->statut == 'actif') ? 'selected' : ''; ?>>Publié (Public)</option>
                        <option value="brouillon" <?php echo ($this->post->statut == 'brouillon') ? 'selected' : ''; ?>>Brouillon (Privé)</option>
                        <option value="inactif" <?php echo ($this->post->statut == 'inactif') ? 'selected' : ''; ?>>Inactif</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group" style="margin-bottom: 30px;">
                ...
                <textarea id="contenu_post_edit" name="contenu" rows="8" style="width: 100%; padding: 15px; border: 1px solid #dce1e6; border-radius: 8px; font-size: 15px; font-family: inherit; resize: vertical; transition: border-color 0.3s;" onfocus="this.style.borderColor='#3498db'" onblur="this.style.borderColor='#dce1e6'"><?php echo htmlspecialchars($this->post->contenu); ?></textarea>
            </div>
            ...
            <div style="display: flex; gap: 15px; margin-top: 20px;">
                ...
                <a href="index.php?controller=post&action=show&id=<?php echo htmlspecialchars($this->post->id_post); ?>" style="flex: 1; padding: 16px; background: #f8f9fa; color: #2c3e50; border: 1px solid #dce1e6; border-radius: 8px; text-align: center; text-decoration: none; font-weight: 600; font-size: 16px; display: flex; align-items: center; justify-content: center; transition: background 0.2s;" onmouseover="this.style.background='#e9ecef'" onmouseout="this.style.background='#f8f9fa'">Annuler</a>
            </div>
        </form>
    </div>
</div>
