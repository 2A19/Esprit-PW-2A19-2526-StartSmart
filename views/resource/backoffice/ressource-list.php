...
<div class="card">
    <div class="card-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h2>Ressources</h2>
            <a href="index.php?controller=resource&action=resourceCreate" class="btn btn-primary btn-small">+ Nouvelle Ressource</a>
        </div>
    </div>
    ...
    <div style="padding: 20px; border-bottom: 1px solid #e0e0e0;">
        <form method="GET" action="index.php" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
            <input type="hidden" name="controller" value="resource"><input type="hidden" name="action" value="resourceList">
            ...
            <?php if (!empty($search) || $sortBy !== 'date'): ?>
                <a href="index.php?controller=resource&action=resourceList" class="btn btn-secondary" style="padding: 8px 15px;">Réinitialiser</a>
            <?php endif; ?>
        </form>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; gap: 5px;">
                                <a href="index.php?controller=resource&action=resourceEdit&id=<?php echo $ressource['id_ressource']; ?>" class="btn btn-info btn-small">
                                    Modifier
                                </a>
                                <a href="index.php?controller=resource&action=resourceDelete&id=<?php echo $ressource['id_ressource']; ?>" class="btn btn-danger btn-small" onclick="return confirm('Êtes-vous sûr?')">
                                    Supprimer
                                </a>
                            </div>
                        </td>
    ...
    <?php else: ?>
        <div class="no-data">
            ...
            <?php if (empty($search) && $sortBy === 'date'): ?>
                <a href="index.php?controller=resource&action=resourceCreate" class="btn btn-primary mt-20">Créer une Ressource</a>
            <?php else: ?>
                <a href="index.php?controller=resource&action=resourceList" class="btn btn-secondary mt-20">Afficher toutes les ressources</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</div>
