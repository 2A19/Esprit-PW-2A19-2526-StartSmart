...
$sections = [
    ...
    'resource-resources' => ['label' => 'Ressources', 'icon' => 'R'],
    'stats' => ['label' => 'Statistiques', 'icon' => 'S'],
];
  ...
  <main class="main">
    ...
    <?php if ($activeSection === 'stats'): ?>
        <div class="stats-grid">
            ...
            <div class="stats-card"><div class="stats-title">Ressources</div><div class="stats-value"><?php echo (int)($stats['ressources'] ?? 0); ?></div></div>
            <div class="stats-card"><div class="stats-title">Demandes ressources</div><div class="stats-value"><?php echo (int)($stats['demandes_ressources'] ?? 0); ?></div></div>
            <div class="stats-card"><div class="stats-title">Sponsors</div><div class="stats-value"><?php echo (int)($stats['sponsors'] ?? 0); ?></div></div>            <div class="stats-card"><div class="stats-title">Offres emploi</div><div class="stats-value"><?php echo (int)($stats['job_offers'] ?? 0); ?></div></div>
    ...
    <?php elseif ($activeSection === 'resource-resources'): ?>
        <div class="card">
            <div class="card-header">
                <div class="card-title">Gestion Ressources</div>
            </div>
            <div class="search-bar">
                <div class="search-input-wrap">
                    <input class="search-input" type="search" placeholder="Rechercher une ressource..." oninput="filterAdminTable(this.value)">
                </div>
                <a class="btn-export" href="index.php?controller=resource&action=ressources">Catalogue public</a>
                <a class="btn-export" href="index.php?controller=resource&action=resourceList">Backoffice ressources</a>
                <a class="btn-export" href="index.php?controller=resource&action=demandeList">Demandes d'acces</a>
                <a class="btn-export" href="index.php?controller=resource&action=sponsorList">Sponsors</a>
                <a class="btn-add" href="index.php?controller=resource&action=resourceCreate">+ Nouvelle ressource</a>
            </div>
            <div class="stats-grid" style="padding: 0 28px 24px; grid-template-columns: repeat(3, minmax(160px, 1fr));">
                <div class="stats-card"><div class="stats-title">Ressources</div><div class="stats-value"><?php echo (int)($stats['ressources'] ?? 0); ?></div></div>
                <div class="stats-card"><div class="stats-title">Demandes</div><div class="stats-value"><?php echo (int)($stats['demandes_ressources'] ?? 0); ?></div></div>
                <div class="stats-card"><div class="stats-title">Sponsors</div><div class="stats-value"><?php echo (int)($stats['sponsors'] ?? 0); ?></div></div>
            </div>
            <div class="table-wrap">
                <table id="adminDataTable">
                    <thead><tr><th>Ressource</th><th>Sponsor</th><th>Type</th><th>Disponibilite</th><th>Statut</th><th>Actions</th></tr></thead>
                    <tbody>
                    <?php if (empty($recentResources)): ?>
                        <tr><td colspan="6" style="color: var(--muted); text-align: center;">Aucune ressource.</td></tr>
                    <?php else: ?>
                            ...
                            <tr>
                                <td>
                                    <div class="avatar-cell">
                                        ...
                                        <div class="name-cell"><strong><?php echo htmlspecialchars($row['nom_ressource'] ?? 'Ressource'); ?></strong></div>
                                    </div>
                                </td>
                                <td style="color: var(--muted);"><?php echo htmlspecialchars($row['nom_sponsor'] ?? 'Sponsor'); ?></td>
                                <td><?php echo htmlspecialchars($row['type_ressource'] ?? ''); ?></td>
                                <td><?php echo (int)($row['quantite_disponible'] ?? 0) - (int)($row['quantite_utilisee'] ?? 0); ?> / <?php echo (int)($row['quantite_disponible'] ?? 0); ?></td>
                                <td><?php echo adminStatusBadge($row['statut'] ?? 'disponible'); ?></td>
                                <td class="actions">
                                    <a class="btn-edit" href="index.php?controller=resource&action=resourceShow&id=<?php echo (int)$row['id_ressource']; ?>">Voir</a>
                                    <a class="btn-ban" href="index.php?controller=resource&action=resourceEdit&id=<?php echo (int)$row['id_ressource']; ?>">Modifier</a>
                                    <a class="btn-delete" href="index.php?controller=resource&action=resourceDelete&id=<?php echo (int)$row['id_ressource']; ?>" onclick="return confirm('Supprimer cette ressource ?');">Supprimer</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        </div>    <?php elseif ($activeSection === 'evenements'): ?>
        <!-- ── Statistics summary ── -->
        <?php
