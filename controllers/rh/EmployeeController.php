...
require_once(__DIR__ . '/../../models/rh/Employee.php');
...
class EmployeeController {
    ...
    public function showEmployee($id) {
        $sql = "SELECT e.*, u.nom_startup AS company_name, jo.title 
                FROM employees e 
    ...
    public function searchEmployees($userId, $keyword = '') {
        ...
        if (!empty($keyword)) {
            $sql .= " AND (full_name LIKE :keyword1 OR email LIKE :keyword2 OR position LIKE :keyword3)";
        }
        ...
        if (!empty($keyword)) {
            ...
            $query->bindValue(':keyword3', '%' . $keyword . '%');
        }
    ...
    public function suggestEmployeesForJob($userId, $jobOffer) {
        $result = $this->listEmployees($userId);
        $employees = [];
        ...
        while ($employee = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($position) {
                if (stripos($jobText, $position) !== false) {
                    $score += 6;
                } else {
                    foreach (preg_split('/\W+/', $position) as $token) {
            ...
            if ($department && stripos($jobText, $department) !== false) {
                $score += 3;
            }
        ...
        return array_slice(array_filter($employees, function($employee) {
            return $employee['match_score'] > 0;
        }), 0, 5);
    }
    ...
    public function create() {
        // Show create employee form (startup only)
        if (empty($_SESSION['user_id']) || ($_SESSION['user_role'] ?? null) !== 'startup') {
            header('Location: login.php');
            exit;
        }

        $data = ['currentView' => 'create-employee'];
        $this->renderView('backend/backend', $data);
    }

    public function edit() {
        // Show edit employee form (startup only)
        if (empty($_SESSION['user_id']) || ($_SESSION['user_role'] ?? null) !== 'startup') {
            header('Location: login.php');
            exit;
        }
        ...
        $id = $_GET['id'] ?? null;
        if (!$id) {
            header('Location: rh.php?page=employee/index');
            exit;
        }

        $employee = $this->showEmployee($id);
        if (!$employee || $employee['user_id'] != $_SESSION['user_id']) {
            header('Location: rh.php?page=employee/index');
            exit;
        }

        $data = ['employee' => $employee, 'currentView' => 'edit-employee'];
        $this->renderView('backend/backend', $data);
    }

    public function view() {
        // Show employee detail (startup only)
        if (empty($_SESSION['user_id']) || ($_SESSION['user_role'] ?? null) !== 'startup') {
            header('Location: login.php');
            exit;
        }
        ...
        $data = ['employee' => $employee, 'currentView' => 'employee-detail'];
        $this->renderView('backend/backend', $data);
    }
