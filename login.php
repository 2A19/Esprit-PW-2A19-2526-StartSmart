...
function handleGoogleCredentialResponse(response) {
  ...
  form.method = 'POST';
  form.action = 'api/index.php?path=auth/google_login';
...
async function captureFaceForLogin() {
  try {
    ...
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = 'api/index.php?path=auth/face_login';
    form.innerHTML = `
...
function doLogin() {
  ...
  if (!ok) return;
  const form = document.createElement('form');
  form.method = 'POST'; form.action = 'api/index.php?path=auth/login';
...
function doRegUser() {
  ...
  fetch('api/index.php?path=auth/register_user',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
...
function doRegStartup() {
  ...
  fd.append('h-captcha-response',captchaToken);
  if(photo)fd.append('profile_picture',photo);
  fetch('api/index.php?path=auth/register_startup',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
    if(d.success){document.getElementById('v-register').innerHTML='<div class="alert success" style="display:block">'+d.message+'</div>';}
...
function fpSendOtp() {
  ...
  fetch('api/index.php?path=auth/forgot_password', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(() => {
...
function fpVerifyOtp() {
  ...
  fetch('api/index.php?path=auth/verify_otp', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(d => {
      if (d.success) {
...
function fpResetPass() {
  ...
  fetch('api/index.php?path=auth/reset_password', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(d => {
