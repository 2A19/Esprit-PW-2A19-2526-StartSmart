...
class ProjetCommentaire {
    ...
    public function readByProjet($projetId) {
        $query = "SELECT c.*, u.nom as auteur_nom, u.prenom as auteur_prenom,
                         (SELECT COUNT(*) FROM projet_reaction pr WHERE pr.commentaire_id = c.id AND pr.type = 'LIKE') as likes_count,
                         (SELECT COUNT(*) FROM projet_reaction pr WHERE pr.commentaire_id = c.id AND pr.type = 'DISLIKE') as dislikes_count
