...
class Reaction {
    ...
    private function getExistingPostReaction() {
        $query = "SELECT id, type FROM " . $this->table_name . " WHERE user_id = :user_id AND post_id = :post_id LIMIT 1";
    ...
    private function getExistingCommentReaction() {
        $query = "SELECT id, type FROM " . $this->table_name . " WHERE user_id = :user_id AND comment_id = :comment_id LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":user_id", $this->user_id);
