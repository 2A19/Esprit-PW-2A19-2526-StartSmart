...
class Reaction {
    ...
    private function getExistingPostReaction() {
        $query = "SELECT id_reaction, type FROM " . $this->table_name . " WHERE user_id = :user_id AND post_id = :post_id LIMIT 1";
    ...
    private function getExistingCommentReaction() {
        $query = "SELECT id_reaction, type FROM " . $this->table_name . " WHERE user_id = :user_id AND comment_id = :comment_id LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":user_id", $this->user_id);
    ...
    public function togglePostReaction($reactionType = 'LIKE') {
        ...
        if ($existing) {
            if (strtoupper($existing['type']) === $this->type) {
                ...
                $deleteStmt->bindParam(1, $existing['id_reaction']);
                if ($deleteStmt->execute()) {
                    return ["status" => "removed", "current_type" => null];
                }
            }

            $updateStmt = $this->conn->prepare("UPDATE " . $this->table_name . " SET type = :type WHERE id_reaction = :id_reaction");
            $updateStmt->bindParam(":type", $this->type);
            $updateStmt->bindParam(":id_reaction", $existing['id_reaction']);
