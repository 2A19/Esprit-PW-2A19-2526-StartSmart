    ...
    public function getConversations($user_id) {
        $query = "
            SELECT 
                m.projet_id,
                p.nomprojet,
                u.id_utilisateur as other_user_id,
                u.nom as other_user_name,
                MAX(m.created_at) as last_message_date,
                SUM(CASE WHEN m.receiver_id = ? AND m.is_read = 0 THEN 1 ELSE 0 END) as unread_count
            FROM " . $this->table_name . " m
            JOIN projet p ON m.projet_id = p.id
            JOIN utilisateur u ON (u.id_utilisateur = m.sender_id OR u.id_utilisateur = m.receiver_id) AND u.id_utilisateur != ?
            WHERE (m.sender_id = ? OR m.receiver_id = ?)
            GROUP BY m.projet_id, p.nomprojet, u.id_utilisateur, u.nom
