    ...
    private function calculateActivityScore($user_id) {
        ...
        $query_avg = "
            SELECT AVG(activity_score) as avg_score FROM (
                ...
                LEFT JOIN reaction pr ON u.id = pr.user_id
                LEFT JOIN commentaire c ON u.id = c.auteur_id
                LEFT JOIN projet_reaction pjr ON u.id = pjr.user_id
                LEFT JOIN projet_commentaire pjc ON u.id = pjc.auteur_id
                GROUP BY u.id
            ) as user_activities
        ";
