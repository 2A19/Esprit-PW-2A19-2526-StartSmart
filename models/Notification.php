...
class Notification {
    ...
    public function getUnreadByUser($userId, $limit = 10) {
        $sql = "SELECT n.*, u.nom as actor_name FROM notification n LEFT JOIN users u ON n.actor_id = u.id WHERE n.user_id = :user_id ORDER BY n.created_at DESC LIMIT :limit";
        $stmt = $this->conn->prepare($sql);
    ...
    public function getSinceByUser($userId, $sinceId = 0, $limit = 50) {
        $sql = "SELECT n.*, u.nom as actor_name FROM notification n LEFT JOIN users u ON n.actor_id = u.id WHERE n.user_id = :user_id AND n.id_notification > :since_id ORDER BY n.id_notification ASC LIMIT :limit";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(':user_id', $userId, PDO::PARAM_INT);
