...
class Projet {
    ...
    public function readAll($search = "", $ownerId = null, $sortBy = "latest", $currentUserId = null, $limit = null, $offset = null) {
        $query = "SELECT p.*, c.typeprojet AS categorie_nom, u.nom AS auteur_nom,
                    COUNT(DISTINCT pc.id) AS commentaires_count,
                    COUNT(DISTINCT pr_like.id) AS likes_count,
                    COUNT(DISTINCT pr_dislike.id) AS dislikes_count";
        
        if ($currentUserId) {
            $query .= ", (SELECT type FROM projet_reaction WHERE projet_id = p.id AND user_id = :current_user_id LIMIT 1) as current_user_reaction ";
        } else {
        ...
        $query .= "FROM " . $this->table_name . " p 
                  ...
                  LEFT JOIN projet_commentaire pc ON pc.projet_id = p.id AND pc.statut = 'actif'
                  LEFT JOIN projet_reaction pr_like ON pr_like.projet_id = p.id AND pr_like.type = 'LIKE'
                  LEFT JOIN projet_reaction pr_dislike ON pr_dislike.projet_id = p.id AND pr_dislike.type = 'DISLIKE'";
    ...
    public function update() {
        $query = "UPDATE " . $this->table_name . " 
                  SET num=:num, nomprojet=:nomprojet, description=:description, datedebut=:datedebut, datefin=:datefin, budget=:budget, gain=:gain, categorie_id=:categorie_id, city=:city, country=:country, latitude=:latitude, longitude=:longitude, statut=:statut, updated_at=NOW()
                  WHERE id = :id";
