...
class Categorie {
    ...
    public function readAll($search = "") {
        $query = "SELECT c.*, u.role AS creator_role, COUNT(p.id) AS projets_count 
                  ...
                  LEFT JOIN utilisateur u ON c.created_by_id = u.id_utilisateur";
        
        if (!empty($search)) {
            $query .= " WHERE c.typeprojet LIKE :search OR c.nom_investisseur LIKE :search";
        }
    ...
    public function readAllForSelect() {
        $query = "SELECT id, typeprojet FROM " . $this->table_name . " ORDER BY typeprojet ASC";
        return $this->conn->query($query);
    }

    public function create() {
        $query = "INSERT INTO " . $this->table_name . " 
                  SET num=:num, typeprojet=:typeprojet, nom_investisseur=:nom_investisseur, created_by_id=:created_by_id";
    ...
    public function update() {
        $query = "UPDATE " . $this->table_name . " 
                  SET num=:num, typeprojet=:typeprojet, nom_investisseur=:nom_investisseur 
                  WHERE id = :id";
    ...
    public function isCreatedByAdmin($categorieId) {
        $query = "SELECT u.role 
                  FROM " . $this->table_name . " c 
                  INNER JOIN utilisateur u ON c.created_by_id = u.id_utilisateur 
                  WHERE c.id = ? LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $categorieId);
        $stmt->execute();
