...
class Post {
    ...
    public function readAll($search = "", $topicFilter = "", $sortBy = "latest", $currentUserId = null, $limit = null, $offset = null) {
        $query = "SELECT p.*, u.nom as auteur_nom, prj.nomprojet as projet_nom, cat.titre as categorie_nom, COUNT(DISTINCT c.id) AS commentaires_count ";
        
        if ($currentUserId) {
            $query .= ", (SELECT type FROM reaction r WHERE r.post_id = p.id AND r.user_id = :current_user_id LIMIT 1) as current_user_reaction ";
        } else {
            $query .= ", NULL as current_user_reaction ";
        }

        $query .= ", (SELECT COUNT(*) FROM reaction r WHERE r.post_id = p.id AND r.type = 'LIKE') AS likes_count ";
        $query .= ", (SELECT COUNT(*) FROM reaction r WHERE r.post_id = p.id AND r.type = 'DISLIKE') AS dislikes_count ";
        ...
        if ($sortBy == "most_commented") {
            ...
            $query .= " ORDER BY likes_count DESC, p.id DESC";
        } else {
    ...
    public function readOne($currentUserId = null) {
        $query = "SELECT p.*, u.nom as auteur_nom, prj.nomprojet as projet_nom, cat.titre as categorie_nom ";
    ...
    public function isOwner() {
        ...
        $query = "SELECT 1 FROM " . $this->table_name . " WHERE id = ? AND auteur_id = ? LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $this->id);
