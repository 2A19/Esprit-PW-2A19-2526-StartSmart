<?php
/**
 * Unified Database Configuration for Integrated StartSmart Application.
 *
 * Connection is resolved in this order:
 * 1) Environment: DB_HOST, DB_PORT, DB_DATABASE (or DB_NAME), DB_USERNAME (or DB_USER), DB_PASSWORD
 * 2) Optional file: config/database.local.php — return an array with any of:
 *    host, port, database (or db_name), username (or user), password, charset
 *
 * Default database name is "startsmart" (setup.sql). The RH merge dump
 * database/startsmartrh.sql uses "startsmart_db" — if you imported only that file,
 * set database in database.local.php or DB_DATABASE so login/RH hit the same DB.
 */
...
class Database {
     ...
     */
    public static function getResolvedConfig(): array {
        if (self::$resolvedConfig !== null) {
            return self::$resolvedConfig;
        }

        $database = self::envString('DB_DATABASE', '');
        if ($database === '') {
            $database = self::envString('DB_NAME', 'startsmart');
        }

        $cfg = [
            'host' => self::envString('DB_HOST', 'localhost'),
            'port' => (int) self::envString('DB_PORT', '3306'),
            'database' => $database,
            'username' => self::envString('DB_USERNAME', self::envString('DB_USER', 'root')),
            'password' => getenv('DB_PASSWORD') !== false ? (string) getenv('DB_PASSWORD') : self::envString('MYSQL_PASSWORD', ''),
            'charset' => self::envString('DB_CHARSET', 'utf8mb4'),
        ];
        ...
        if (is_readable($localPath)) {
            ...
            if (is_array($local)) {
                foreach (['host', 'port', 'database', 'db_name', 'username', 'user', 'password', 'charset'] as $k) {
                    ...
                    if ($k === 'port') {
                        ...
                        $cfg['database'] = (string) $val;
                    } elseif ($k === 'user') {
                        $cfg['username'] = (string) $val;
                    } elseif (in_array($k, ['host', 'database', 'username', 'password', 'charset'], true)) {
                        $cfg[$k] = (string) $val;
                    }
                }
            }
    ...
    public function getConnection() {
        ...
        $c = self::getResolvedConfig();

        $connect = function (array $cfg): PDO {
            $dsn = sprintf(
        ...
        try {
            ...
            $unknownDb = stripos($message, 'Unknown database') !== false;
            if ($unknownDb) {
                $fallbackDb = $c['database'] === 'startsmart' ? 'startsmart_db' : 'startsmart';
                $fallback = $c;
                $fallback['database'] = $fallbackDb;

                try {
                    $this->pdo = $connect($fallback);
                    self::$resolvedConfig = $fallback;
                    return $this->pdo;
                } catch (PDOException $fallbackException) {
                    throw new RuntimeException(
                        'Database connection failed (' . $fallback['host'] . ':' . $fallback['port'] . '/' . $fallback['database'] . '): ' . $fallbackException->getMessage()
                    );
                }
            }

            throw new RuntimeException(
                'Database connection failed (' . $c['host'] . ':' . $c['port'] . '/' . $c['database'] . '): ' . $e->getMessage()
            );
        }
    }
    ...
    private function syncSchema() {
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS categorie (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS projet (
            id INT(11) AUTO_INCREMENT PRIMARY KEY,
            num INT(11) NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS post (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS commentaire (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS piece_jointe (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS reaction (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS notification (
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS competence (
        ...
        if (!$this->columnExists('competence', 'category')) {
            $this->getConnection()->exec("ALTER TABLE competence ADD COLUMN category VARCHAR(50) NULL");
        }
        if (!$this->columnExists('competence', 'description')) {
            $this->getConnection()->exec("ALTER TABLE competence ADD COLUMN description TEXT NULL");
        }

        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS utilisateur_competence (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS projet_competence (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS utilisateur_interet (
            id INT AUTO_INCREMENT PRIMARY KEY,
            utilisateur_id INT NOT NULL,
        ...
        if (!$this->columnExists('utilisateur_interet', 'interest_score')) {
            $this->getConnection()->exec("ALTER TABLE utilisateur_interet ADD COLUMN interest_score INT DEFAULT 1");
        }

        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS projet_commentaire (
            id INT AUTO_INCREMENT PRIMARY KEY,
            projet_id INT NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS projet_reaction (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            projet_id INT NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS skill (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS user_skill (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS project_skill (
            id INT AUTO_INCREMENT PRIMARY KEY,
            projet_id INT NOT NULL,
            skill_id INT NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS user_interest (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            categorie_id INT NOT NULL,
            interest_score INT DEFAULT 1,
        ...
        if (!$this->columnExists('user_interest', 'interest_score')) {
            $this->getConnection()->exec("ALTER TABLE user_interest ADD COLUMN interest_score INT DEFAULT 1");
        }

        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS project_match_cache (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            projet_id INT NOT NULL,
        ...
        $this->getConnection()->exec("CREATE TABLE IF NOT EXISTS user_match_action (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            projet_id INT NOT NULL,
        ...
        $this->getConnection()->exec("INSERT INTO categorie (id, num, typeprojet, nom_investisseur, created_by_id)
            VALUES
        ...
        $this->getConnection()->exec("INSERT IGNORE INTO skill (name, category, description) VALUES " . implode(', ', $values));
        ...
        $this->getConnection()->exec("INSERT IGNORE INTO competence (nom, category, description) VALUES " . implode(', ', $skillValues));

        $this->getConnection()->exec("INSERT IGNORE INTO utilisateur_competence (utilisateur_id, competence_id) VALUES (1, 1), (2, 2)");
        $this->getConnection()->exec("INSERT IGNORE INTO projet_competence (projet_id, competence_id) VALUES (1, 1)");
        $this->getConnection()->exec("INSERT IGNORE INTO utilisateur_interet (utilisateur_id, categorie_id, interest_score) VALUES (1, 1, 3), (2, 2, 2)");
        $this->getConnection()->exec("INSERT IGNORE INTO user_skill (user_id, skill_id, proficiency_level) VALUES (1, 1, 'expert'), (2, 2, 'intermediate')");
        $this->getConnection()->exec("INSERT IGNORE INTO project_skill (projet_id, skill_id, required, priority) VALUES (1, 1, 1, 1)");
    }
}

// Compatibility shim for imported StartSmartrh controllers.
// Original RH code calls config::getConnexion(); keep that API intact.
